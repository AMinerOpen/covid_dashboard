...
export default class AlgorithmPage extends React.Component<IProps, IState> {
  ...
  renderAlgorithms() {
    let al1: HTMLElement | null = document.getElementById('al1');
    if(al1) {
      katex.render("RI(t) = RI_{prior} \\times RI_{posterior}(t)", al1,
        {throwOnError: false, output: "html"});
    }
    let al2: HTMLElement | null = document.getElementById('al2');
    if(al2) {
      katex.render("\\frac{RI_{posterior}(t)}{1-RI_{posterior}(t)} = \\sigma(w \\cdot v(t))", al2,
        {throwOnError: false, output: "html"});
    }
    let al3: HTMLElement | null = document.getElementById('al3');
    if(al3) {
      katex.render("RI_{posterior}(t) = \\frac{\\sigma(w \\cdot v(t))}{1+\\sigma(w \\cdot v(t))}", al3,
        {throwOnError: false, output: "html"});
    }
    let al4: HTMLElement | null = document.getElementById('al4');
    if(al4) {
      katex.render("\\frac{dN(t)}{dt} = \\gamma(t)N(t)", al4,
        {throwOnError: false, output: "html"});
    }
    let al5: HTMLElement | null = document.getElementById('al5');
    if(al5) {
      katex.render("\\gamma(t) = at + b", al5,
        {throwOnError: false, output: "html"});
    }
    let al6: HTMLElement | null = document.getElementById('al6');
    if(al6) {
      katex.render("N(t) = N(0)e^{\\frac{a}{2}t^{2}+bt}", al6,
        {throwOnError: false, output: "html"});
    }
    let al7: HTMLElement | null = document.getElementById('al7');
    if(al7) {
      katex.render("t^{*} = -\\frac{b}{a}", al7,
        {throwOnError: false, output: "html"});
    }
    let al8: HTMLElement | null = document.getElementById('al8');
    if(al8) {
      katex.render("v(t) = (t^{*} - t,\\gamma(t),N(t) \\cdot D(t),\\frac{N(t)}{P})^{T}", al8,
        {throwOnError: false, output: "html"});
    }
  }
